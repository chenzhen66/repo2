...
<head>
    ...
    <script>
        ${msg}
        function shangCai(value) {
            let id=value.split(",")[0];
            let code = value.split(",")[1]
            location.href="AdminOrderServlet?action=shangcai&id="+id+"&code="+code;
        }
    </script>
</head>
<body>
            ...
            <c:forEach items="${OrderDetails}" var="item">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.status==0}">
                                <button class="btn btn-info" onclick="shangCai(this.value)" value="${item.id},${item.orders.code}">上菜</button>
                            </c:when>
                            <c:when test="${item.status==0}">
                                <button class="btn btn-info" onclick="shangCai(this.value)" value="${item.id},${item.orders.code}">上菜</button>
                            </c:when>
